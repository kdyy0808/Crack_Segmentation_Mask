...
common_path = "/home/pirl/DH/pirl/CRACK_SEG/Crack_img_data/3rd_training/test/"
...
for file_name in file_list:
    if file_name in gt_file_list:
        if file_name in predict_file_list:
            ...
            # 이미지에 글자 합성하기
            img[:200,:660] = (255,255,255)
            img = cv2.putText(img, "Precision : {:.3f}".format(Precision), (5, 25+20), font, 3, blue, 1)
            img = cv2.putText(img, "Recall    : {:.3f}".format(Recall), (5, 75+20), font, 3, blue, 1)
            img = cv2.putText(img, "IOU       : {:.3f}".format(IOU), (5, 125+20), font, 3, blue, 1)
